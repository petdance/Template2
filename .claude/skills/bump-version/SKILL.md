...
- POD update is best-effort: it looks for a line of the form `<prose> version X.YY[, released on <date>].` inside the `=head1 VERSION` section. If nothing matches, it reports `no matching prose line` and leaves the file alone — fix the POD manually in that case.
